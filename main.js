...
(function() {
	...
	*/
	...
	function init() {
		...
		// Register event listeners
		$('login-btn').addEventListener('click', login);
        $('preregister-btn').addEventListener('click', doRegister);
        $('register-btn').addEventListener('click', signUp);
		$('nearby-btn').addEventListener('click', loadNearbyItems);
		$('fav-btn').addEventListener('click', loadFavoriteItems);
		$('recommend-btn').addEventListener('click', loadRecommendedItems);
		$('back-btn').addEventListener('click', validateSession);
	...
	function login() {
		...
		setTimeout(function(){ $('login-error').innerHTML = '&nbsp;'; }, 2000);
	}
	...
	function signUp() {
        ...
        if (newpassword != cpassword) {
          confirmPasswordError();
			...
			newpassword = md5(newusername + md5(newpassword));
			...
			function(res) {
				...
				// successfully logged in
				if (result.status === 'OK') {
					$('password-error').innerHTML = 'Sign Up Success!';
					setTimeout(function() {
						onSessionValid(result);
					});
					//onSessionValid(result);
        ...
        setTimeout(function(){ $('register-error').innerHTML = '&nbsp;'; $('password-error').innerHTML = '&nbsp;'; }, 2000);
    }
  
    function showSignUpError() {
		$('register-error').innerHTML = 'Invalid username';
	}
  
    function confirmPasswordError() {
		$('password-error').innerHTML = 'Inconsistent password';
	}
